...
export default function EnterpriseOAuthManagementPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    try {
      const response = await fetch('/api/enterprise-admin/enterprise-oauth', {
        credentials: 'include',
      })
  ...
  const handleSaveConfig = async () => {
    ...
    const isUpdatingSecret =
      formData.custom_client_secret !== '••••••••' && formData.custom_client_secret.trim() !== ''
  ...
  const handleDeleteConfig = async (tenantId: string) => {
    ...
    try {
      const response = await fetch(`/api/enterprise-admin/enterprise-oauth?provider=google_sheets`, {
        method: 'DELETE',
        credentials: 'include',
      })
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-500 dark:text-gray-400">
            This page is only accessible to Enterprise tenant administrators.
          </p>
        </div>
      </div>
    )
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
              Enterprise OAuth (BYO) Settings
            </h1>
            <p className="mt-1 text-gray-500 dark:text-gray-400">
              Configure your tenant’s Google OAuth credentials and DWD subject email.
            </p>
          </div>
          <button
            onClick={loadData}
          ...
          {enterpriseTenants.length === 0 ? (
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-8 text-center dark:border-gray-700 dark:bg-gray-800">
              ...
              <p className="mt-4 text-gray-500 dark:text-gray-400">
                No enterprise tenant found for your user.
              </p>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    <div className="mb-4 flex items-start justify-between">
                      ...
                      >
                        {config?.use_custom_credentials ? 'BYO Enabled' : 'Platform OAuth'}
                      </span>
                    </div>
                    ...
                    {config ? (
                      <div className="mb-4 space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className={`text-xs font-medium ${config.is_enabled ? 'text-green-600' : 'text-gray-500'}`}>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                  ...
                  {oauthConfigs.find((c) => c.tenant_id === selectedTenant?.id)
                ...
                <div className="relative mt-1">
                  <input
                    ...
                    placeholder={
                      formData.custom_client_secret === '••••••••'
                        ? 'Existing secret (enter new to update)'
                        : 'Enter client secret'
                    }
                  ...
                  >
                    {showSecrets['modal'] ? <EyeSlashIcon className="h-5 w-5" /> : <EyeIcon className="h-5 w-5" />}
                  </button>
                </div>
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                  Domain-Wide Delegation Subject Email (recommended for BYO)
                </label>
                <input
                  type="email"
                ...
                />
                <p className="mt-1 text-xs text-gray-500">
                  Used for service-account impersonation (DWD) in Google Workspace.
                </p>
              </div>
