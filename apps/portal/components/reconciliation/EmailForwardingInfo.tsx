  ...
  const loadEmailAddress = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setStats(data.statistics || { total: 0, completed: 0, failed: 0, pending: 0 })
      } else {
        const errorData = await response.json().catch(() => ({}))
        console.error('Failed to load email address:', errorData.error || 'Unknown error')
        // Don't set error state here - let the UI show "No Address Yet" instead
      }
    } catch (error) {
  ...
  const generateAddress = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json().catch(() => ({}))
        console.error('Failed to generate address:', errorData.error || 'Unknown error')
        alert(errorData.error || 'Failed to generate email address. Please try again.')
      }
    } catch (error) {
